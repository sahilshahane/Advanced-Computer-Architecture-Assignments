...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
...
#ifdef SANITY_CHECK
        if (MSHR.next_fill_index >= MSHR_SIZE)
            assert(0);
#endif

        uint32_t mshr_index = MSHR.next_fill_index;

        // NOTE: Initial invalidation removed - we'll do it at the right time below
        ...
        if (cache_type == IS_ITLB || cache_type == IS_DTLB || cache_type == IS_STLB)
        {
            ...
            if (MSHR.entry[mshr_index].data == (UINT64_MAX >> LOG2_PAGE_SIZE))
            {
                ...
                if (MSHR.entry[mshr_index].type == TRANSLATION_FROM_L1D || MSHR.entry[mshr_index].type == PREFETCH_TRANSLATION)
                {
                    ...
                    if (cache_type == IS_STLB && MSHR.entry[mshr_index].l1_pq_index != -1 && MSHR.entry[mshr_index].fill_l1d != -1)
                    {
                    ...
                    else if (cache_type == IS_STLB && MSHR.entry[mshr_index].prefetch_translation_merged && MSHR.entry[mshr_index].fill_l1d != -1)
                    {
                        assert(MSHR.entry[mshr_index].type == TRANSLATION_FROM_L1D);
                        PACKET temp = MSHR.entry[mshr_index];
                        temp.data_pa = MSHR.entry[mshr_index].data;
        ...
        // EXCLUSIVE CACHE: Check if victim is dirty
        if (block[set][way].dirty)
        {
            // check if the lower level WQ has enough room
            if (lower_level)
            {
                if (lower_level->get_occupancy(2, block[set][way].address) == lower_level->get_size(2, block[set][way].address))
                {
                    ...
                    DP(if (warmup_complete[fill_cpu]) {
                            ...
                            cout << " victim_addr: " << block[set][way].address << dec << endl; });
                }
                else
                {
                    // EXCLUSIVE CACHE: Send victim to lower level (both dirty and clean)
                    ...
                    writeback_packet.type = WRITEBACK; //WRITEBACK
                    writeback_packet.event_cycle = current_core_cycle[fill_cpu];

                    // Preserve metadata
                    writeback_packet.instruction = block[set][way].instruction;
                    writeback_packet.is_data = (cache_type == IS_L1D) ? 1 : 0;
            ...
            {
                if (cache_type != IS_STLB && cache_type != IS_LLC)
                    assert(0);
            }
#endif
        }
        ...
        if (do_fill)
        {
            //@Vasudha: For PC-offset DTLB prefetcher
            ...
            // EXCLUSIVE CACHE: Enhanced bypass logic
            uint8_t bypass_fill = 0;

            // Check if data will be immediately sent to upper level
            if (MSHR.entry[mshr_index].fill_level < fill_level)
            {
                // Data is going upward - bypass fill at current level for exclusivity
                bypass_fill = 1;
            }
            else if (cache_type == IS_L2C)
            {
                // Additional checks for L2C
                if (MSHR.entry[mshr_index].send_both_cache ||
                    MSHR.entry[mshr_index].fill_l1i ||
                    MSHR.entry[mshr_index].fill_l1d)
                {
                    bypass_fill = 1;
                }
            }
            // EXCLUSIVE CACHE: Handle bypass case - invalidate before forwarding
            if (bypass_fill && cache_type == IS_L2C)
            {
                // L2C is bypassing - invalidate L2C's own copy and LLC before forwarding to L1
                PACKET inv_packet;
                inv_packet.address = MSHR.entry[mshr_index].address;
                inv_packet.full_addr = MSHR.entry[mshr_index].full_addr;
                inv_packet.full_physical_address = MSHR.entry[mshr_index].full_addr;

                // Invalidate L2C's own copy (all ways in the set)
                for (uint32_t l2_way = 0; l2_way < NUM_WAY; l2_way++)
                {
                    if (block[set][l2_way].valid &&
                        block[set][l2_way].address == inv_packet.address)
                    {
                        block[set][l2_way].valid = 0;
                        block[set][l2_way].dirty = 0;
                        break;
                    }
                }

                // Invalidate LLC
                uint32_t llc_set = uncore.LLC.get_set(inv_packet.address);
                for (uint32_t llc_way = 0; llc_way < uncore.LLC.NUM_WAY; llc_way++)
                {
                    if (uncore.LLC.block[llc_set][llc_way].valid &&
                        uncore.LLC.block[llc_set][llc_way].address == inv_packet.address)
                    {
                        uncore.LLC.block[llc_set][llc_way].valid = 0;
                        uncore.LLC.block[llc_set][llc_way].dirty = 0;
                        break;
                    }
                }
            }

            // Fill cache only if not bypassing
            if (!bypass_fill)
            {
                // EXCLUSIVE CACHE: RIGHT BEFORE FILLING, invalidate all other levels
                // This is the CRITICAL step - must happen just before fill_cache()
                PACKET inv_final;
                inv_final.address = MSHR.entry[mshr_index].address;
                inv_final.full_addr = MSHR.entry[mshr_index].full_addr;
                inv_final.full_physical_address = MSHR.entry[mshr_index].full_addr;

                // Invalidate from LLC (unless we ARE LLC)
                if (cache_type != IS_LLC)
                {
                    uint32_t llc_set = uncore.LLC.get_set(inv_final.address);
                    for (uint32_t llc_way = 0; llc_way < uncore.LLC.NUM_WAY; llc_way++)
                    {
                        if (uncore.LLC.block[llc_set][llc_way].valid &&
                            uncore.LLC.block[llc_set][llc_way].address == inv_final.address)
                        {
                            uncore.LLC.block[llc_set][llc_way].valid = 0;
                            uncore.LLC.block[llc_set][llc_way].dirty = 0;
                            break;
                ...
                // Invalidate from L2C (unless we ARE L2C)
                if (cache_type != IS_L2C)
                {
                    uint32_t l2_set = ooo_cpu[fill_cpu].L2C.get_set(inv_final.address);
                    for (uint32_t l2_way = 0; l2_way < ooo_cpu[fill_cpu].L2C.NUM_WAY; l2_way++)
                    {
                        if (ooo_cpu[fill_cpu].L2C.block[l2_set][l2_way].valid &&
                            ooo_cpu[fill_cpu].L2C.block[l2_set][l2_way].address == inv_final.address)
                        {
                            ooo_cpu[fill_cpu].L2C.block[l2_set][l2_way].valid = 0;
                            ooo_cpu[fill_cpu].L2C.block[l2_set][l2_way].dirty = 0;
                            break;
                        }
                    }
                }

                // Invalidate from L1I (unless we ARE L1I)
                if (cache_type != IS_L1I)
                {
                    uint32_t l1i_set = ooo_cpu[fill_cpu].L1I.get_set(inv_final.address);
                    for (uint32_t l1i_way = 0; l1i_way < ooo_cpu[fill_cpu].L1I.NUM_WAY; l1i_way++)
                    {
                        if (ooo_cpu[fill_cpu].L1I.block[l1i_set][l1i_way].valid &&
                            ooo_cpu[fill_cpu].L1I.block[l1i_set][l1i_way].address == (inv_final.full_physical_address >> LOG2_BLOCK_SIZE))
                        {
                            ooo_cpu[fill_cpu].L1I.block[l1i_set][l1i_way].valid = 0;
                            ooo_cpu[fill_cpu].L1I.block[l1i_set][l1i_way].dirty = 0;
                            break;
                        }
                    }
                }

                // Invalidate from L1D (unless we ARE L1D)
                if (cache_type != IS_L1D)
                {
                    uint32_t l1d_set = ooo_cpu[fill_cpu].L1D.get_set(inv_final.address);
                    for (uint32_t l1d_way = 0; l1d_way < ooo_cpu[fill_cpu].L1D.NUM_WAY; l1d_way++)
                    {
                        if (ooo_cpu[fill_cpu].L1D.block[l1d_set][l1d_way].valid &&
                            ooo_cpu[fill_cpu].L1D.block[l1d_set][l1d_way].address == (inv_final.full_physical_address >> LOG2_BLOCK_SIZE))
                        {
                            ooo_cpu[fill_cpu].L1D.block[l1d_set][l1d_way].valid = 0;
                            ooo_cpu[fill_cpu].L1D.block[l1d_set][l1d_way].dirty = 0;
                            break;
                        }
                    }
                }
            ...
            // STLB processed queue handling
            if (cache_type == IS_STLB && MSHR.entry[mshr_index].l1_pq_index != -1 && (MSHR.entry[mshr_index].send_both_tlb or !MSHR.entry[mshr_index].instruction))
            {
                PACKET temp = MSHR.entry[mshr_index];
                temp.data_pa = bypass_fill ? MSHR.entry[mshr_index].data : block[set][way].data;
                assert(temp.l1_rq_index == -1 && temp.l1_wq_index == -1);
                temp.read_translation_merged = 0;
                temp.write_translation_merged = 0;
                if (PROCESSED.occupancy < PROCESSED.SIZE)
            ...
            else if (cache_type == IS_STLB && MSHR.entry[mshr_index].prefetch_translation_merged)
            {
                PACKET temp = MSHR.entry[mshr_index];
                temp.data_pa = bypass_fill ? MSHR.entry[mshr_index].data : block[set][way].data;
                temp.read_translation_merged = 0;
            ...
            if (cache_type == IS_STLB)
            {
                ...
                {
                    stlb_hints_to_l2++;
                    uint64_t data_for_hint = bypass_fill ? MSHR.entry[mshr_index].data : block[set][way].data;
                    uint64_t phy_addr = ((data_for_hint << LOG2_PAGE_SIZE) | (ooo_cpu[fill_cpu].L1D.RQ.entry[MSHR.entry[mshr_index].l1_rq_index].full_addr & ((1 << LOG2_PAGE_SIZE) - 1))) >> LOG2_BLOCK_SIZE;

                    DP(if (warmup_complete[fill_cpu]) {
                            cout << "[" << NAME << "] " << __func__ << " sending stlb hint to L2: ";
                            cout << " phy_addr: " << hex << phy_addr;
                            cout << " ip: " << MSHR.entry[mshr_index].ip << dec << endl; });
            ...
            if ((cache_type == IS_L2C) && (MSHR.entry[mshr_index].type == PREFETCH) && (MSHR.entry[mshr_index].fill_level == FILL_L2))
            {
                if (((MSHR.entry[mshr_index].pf_metadata >> 16) & 1) == 1)
                {
                    ...
                    // Don't fill in L2 since we're pushing to L1
                    bypass_fill = 1;
                }
            }
#endif

            // check fill level and return data to upper level
            if (MSHR.entry[mshr_index].fill_level < fill_level)
            {
                if (cache_type == IS_STLB)
                {
            ...
            {
                MSHR.entry[mshr_index].instruction_pa = bypass_fill ? MSHR.entry[mshr_index].data : block[set][way].data;
                if (PROCESSED.occupancy < PROCESSED.SIZE)
                    PROCESSED.add_queue(&MSHR.entry[mshr_index]);
            }
            else if ((cache_type == IS_DTLB) && (MSHR.entry[mshr_index].type != PREFETCH_TRANSLATION))
            {
                MSHR.entry[mshr_index].data_pa = bypass_fill ? MSHR.entry[mshr_index].data : block[set][way].data;
                if (PROCESSED.occupancy < PROCESSED.SIZE)
                    PROCESSED.add_queue(&MSHR.entry[mshr_index]);
            }
...
void CACHE::handle_writeback()
{
    ...
    multimap<uint64_t, uint32_t> writes_ready;
    assert(cache_type != IS_L1I || cache_type != IS_STLB);
    ...
    if (cache_type == IS_L1D)
        WQ.head = writes_ready_it->second;
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0))
    {
        ...
        { // writeback miss
            ...
            { // RFO miss
                ...
                int mshr_index = check_nonfifo_queue(&MSHR, &WQ.entry[index], false);
                ...
                if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE))
                {
                    ...
                    add_nonfifo_queue(&MSHR, &new_packet);
            ...
            else
            {
                ...
                if (do_fill)
                {
                    ...
                    // ***** EXCLUSIVE CACHE: INVALIDATE BEFORE FILL *****
                    // This is the critical fix for exclusivity violations
                    PACKET inv_packet;
                    inv_packet.address = WQ.entry[index].address;
                    inv_packet.full_addr = WQ.entry[index].full_addr;
                    inv_packet.full_physical_address = WQ.entry[index].full_addr;
                    ...
                    if (cache_type != IS_L2C)
                    {
                        uint32_t l2_set = ooo_cpu[writeback_cpu].L2C.get_set(inv_packet.address);
                        for (uint32_t l2_way = 0; l2_way < ooo_cpu[writeback_cpu].L2C.NUM_WAY; l2_way++)
                        {
                            if (ooo_cpu[writeback_cpu].L2C.block[l2_set][l2_way].valid &&
                                ooo_cpu[writeback_cpu].L2C.block[l2_set][l2_way].address == inv_packet.address)
                            {
                                ooo_cpu[writeback_cpu].L2C.block[l2_set][l2_way].valid = 0;
                                ooo_cpu[writeback_cpu].L2C.block[l2_set][l2_way].dirty = 0;
                                break;
                            }
                        }
                    }
                    ...
                    if (cache_type != IS_L1I)
                    {
                        uint32_t l1i_set = ooo_cpu[writeback_cpu].L1I.get_set(inv_packet.address);
                        for (uint32_t l1i_way = 0; l1i_way < ooo_cpu[writeback_cpu].L1I.NUM_WAY; l1i_way++)
                        {
                            if (ooo_cpu[writeback_cpu].L1I.block[l1i_set][l1i_way].valid &&
                                ooo_cpu[writeback_cpu].L1I.block[l1i_set][l1i_way].address == (inv_packet.full_physical_address >> LOG2_BLOCK_SIZE))
                            {
                                ooo_cpu[writeback_cpu].L1I.block[l1i_set][l1i_way].valid = 0;
                                ooo_cpu[writeback_cpu].L1I.block[l1i_set][l1i_way].dirty = 0;
                                break;
                            }
                        }
                    }
                    ...
                    if (cache_type != IS_L1D)
                    {
                        uint32_t l1d_set = ooo_cpu[writeback_cpu].L1D.get_set(inv_packet.address);
                        for (uint32_t l1d_way = 0; l1d_way < ooo_cpu[writeback_cpu].L1D.NUM_WAY; l1d_way++)
                        {
                            if (ooo_cpu[writeback_cpu].L1D.block[l1d_set][l1d_way].valid &&
                                ooo_cpu[writeback_cpu].L1D.block[l1d_set][l1d_way].address == (inv_packet.full_physical_address >> LOG2_BLOCK_SIZE))
                            {
                                ooo_cpu[writeback_cpu].L1D.block[l1d_set][l1d_way].valid = 0;
                                ooo_cpu[writeback_cpu].L1D.block[l1d_set][l1d_way].dirty = 0;
                                break;
                            }
                        }
                    }
...
void CACHE::handle_read()
{
    ...
    for (uint32_t i = 0; i < MAX_READ; i++)
    {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
        {
            ...
            if (way >= 0)
            { // read hit
                PACKET exclusivity_check;
                exclusivity_check.address = RQ.entry[index].address;
                exclusivity_check.full_addr = RQ.entry[index].full_addr;

                // CRITICAL: For L1I and L1D checks, need physical address
                if (cache_type == IS_L1I || cache_type == IS_L1D)
                {
                    // Current cache is L1, use its physical address for checking other caches
                    exclusivity_check.full_physical_address = RQ.entry[index].full_physical_address;
                    exclusivity_check.address = exclusivity_check.full_physical_address >> LOG2_BLOCK_SIZE;
                }
                else if (cache_type == IS_L2C || cache_type == IS_LLC)
                {
                    // Current cache is L2/LLC, construct physical address for L1 checks
                    // L2/LLC use physical addresses directly
                    exclusivity_check.full_physical_address = RQ.entry[index].full_addr;
                    exclusivity_check.address = RQ.entry[index].address;
                }

                int exclusivity_violation = 0;

                if (cache_type == IS_L1D)
                {
                    // Check L1I
                    PACKET l1i_check = exclusivity_check;
                    l1i_check.full_physical_address = exclusivity_check.full_physical_address;
                    int l1i_way = ooo_cpu[read_cpu].L1I.check_hit(&l1i_check);
                    if (l1i_way >= 0)
                    {
                        exclusivity_violation++;
                        cout << "L1D -> L1I: addr=" << hex << exclusivity_check.address << dec << endl;
                    }

                    // Check L2C (L2C uses physical address, no full_physical_address needed)
                    PACKET l2c_check;
                    l2c_check.address = exclusivity_check.address;
                    l2c_check.full_addr = exclusivity_check.full_physical_address;
                    int l2c_way = ooo_cpu[read_cpu].L2C.check_hit(&l2c_check);
                    if (l2c_way >= 0)
                    {
                        exclusivity_violation++;
                        cout << "L1D -> L2C: addr=" << hex << exclusivity_check.address << dec << endl;
                    }

                    // Check LLC
                    PACKET llc_check;
                    llc_check.address = exclusivity_check.address;
                    llc_check.full_addr = exclusivity_check.full_physical_address;
                    int llc_way = uncore.LLC.check_hit(&llc_check);
                    if (llc_way >= 0)
                    {
                        exclusivity_violation++;
                        cout << "L1D -> LLC: addr=" << hex << exclusivity_check.address << dec << endl;
                    }
                }
                else if (cache_type == IS_L1I)
                {
                    // Check L1D
                    PACKET l1d_check = exclusivity_check;
                    l1d_check.full_physical_address = exclusivity_check.full_physical_address;
                    int l1d_way = ooo_cpu[read_cpu].L1D.check_hit(&l1d_check);
                    if (l1d_way >= 0)
                    {
                        exclusivity_violation++;
                        cout << "L1I -> L1D: addr=" << hex << exclusivity_check.address << dec << endl;
                    }
                    ...
                    {
                        exclusivity_violation++;
                        cout << "L1I -> L2C: addr=" << hex << exclusivity_check.address << dec << endl;
                    }
                    ...
                    {
                        exclusivity_violation++;
                        cout << "L1I -> LLC: addr=" << hex << exclusivity_check.address << dec << endl;
                    }
                }
                else if (cache_type == IS_L2C)
                {
                    // Check L1I (need to set full_physical_address)
                    PACKET l1i_check;
                    l1i_check.address = exclusivity_check.address;
                    l1i_check.full_addr = exclusivity_check.full_addr;
                    l1i_check.full_physical_address = exclusivity_check.full_addr; // L2 full_addr IS physical
                    int l1i_way = ooo_cpu[read_cpu].L1I.check_hit(&l1i_check);
                    if (l1i_way >= 0)
                    {
                        exclusivity_violation++;
                        cout << "L2C -> L1I: addr=" << hex << exclusivity_check.address << dec << endl;
                    }

                    // Check L1D (need to set full_physical_address)
                    PACKET l1d_check;
                    l1d_check.address = exclusivity_check.address;
                    l1d_check.full_addr = exclusivity_check.full_addr;
                    l1d_check.full_physical_address = exclusivity_check.full_addr; // L2 full_addr IS physical
                    int l1d_way = ooo_cpu[read_cpu].L1D.check_hit(&l1d_check);
                    if (l1d_way >= 0)
                    {
                        exclusivity_violation++;
                        cout << "L2C -> L1D: addr=" << hex << exclusivity_check.address << dec << endl;
                    }

                    // Check LLC
                    PACKET llc_check;
                    llc_check.address = exclusivity_check.address;
                    llc_check.full_addr = exclusivity_check.full_addr;
                    int llc_way = uncore.LLC.check_hit(&llc_check);
                    if (llc_way >= 0)
                    {
                        exclusivity_violation++;
                        cout << "L2C -> LLC: addr=" << hex << exclusivity_check.address << dec << endl;
                    }
                }
                else if (cache_type == IS_LLC)
                {
                    ...
                    l1i_check.address = exclusivity_check.address;
                    l1i_check.full_addr = exclusivity_check.full_addr;
                    l1i_check.full_physical_address = exclusivity_check.full_addr; // LLC full_addr IS physical
                    int l1i_way = ooo_cpu[read_cpu].L1I.check_hit(&l1i_check);
                    if (l1i_way >= 0)
                    {
                        exclusivity_violation++;
                        cout << "LLC -> L1I: addr=" << hex << exclusivity_check.address << dec << endl;
                    }
                    ...
                    l1d_check.address = exclusivity_check.address;
                    l1d_check.full_addr = exclusivity_check.full_addr;
                    l1d_check.full_physical_address = exclusivity_check.full_addr; // LLC full_addr IS physical
                    int l1d_way = ooo_cpu[read_cpu].L1D.check_hit(&l1d_check);
                    if (l1d_way >= 0)
                    {
                        exclusivity_violation++;
                        cout << "LLC -> L1D: addr=" << hex << exclusivity_check.address << dec << endl;
                    }

                    // Check L2C
                    PACKET l2c_check;
                    l2c_check.address = exclusivity_check.address;
                    l2c_check.full_addr = exclusivity_check.full_addr;
                    int l2c_way = ooo_cpu[read_cpu].L2C.check_hit(&l2c_check);
                    if (l2c_way >= 0)
                    {
                        exclusivity_violation++;
                        cout << "LLC -> L2C: addr=" << hex << exclusivity_check.address << dec << endl;
                    }
                }
                if (exclusivity_violation > 0)
                {
                    // cout << "It seems exclusivity is violated. Exiting!!" << endl;
                    // assert(0);
                }
                if (cache_type == IS_ITLB)
                {
                ...
                if (RQ.entry[index].type == LOAD)
                {
                    ...
                    { // Neelu: for dense region, only invoking on loads, check other l2c_pref_operate as well.
                        ...
                        //    block[set][way].prefetch = 0; // count once
                            l2c_prefetcher_useful(block[set][way].address<<LOG2_BLOCK_SIZE);
                        }
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
        {
            ...
            { // prefetch hit
                ...
                if (PQ.entry[index].pf_origin_level < fill_level)
                {
                    ...
                    else if ((cache_type == IS_L2C) && (RQ.entry[index].type != PREFETCH_TRANSLATION) && (RQ.entry[index].instruction == 0) && (RQ.entry[index].type != LOAD_TRANSLATION) && (RQ.entry[index].type != TRANSLATION_FROM_L1D))
                    {
                        if (block[set][way].prefetch) {
                            // block[set][way].prefetch = 0; // count once
                            l2c_prefetcher_useful(block[set][way].address<<LOG2_BLOCK_SIZE);
